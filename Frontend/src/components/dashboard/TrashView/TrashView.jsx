...
const TrashView = () => {
  ...
  const fetchTrash = async () => {
    try {
      ...
      const res = await axios.get("http://13.201.87.205:5050/api/trash", {
        headers: {
  ...
  const handleRestore = async () => {
    try {
      ...
      for (const id of selectedIds) {
        await axios.put(
          `http://13.201.87.205:5050/api/trash/recover/${id}`,
          {},
          {
            headers: {
              Authorization: `Bearer ${token}`
  ...
  const handleEmptyTrash = async () => {
    try {
      ...
      await axios.delete("http://13.201.87.205:5050/api/trash", {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
